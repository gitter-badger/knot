...
static int ixfr_load_changes(changeset_t *ch, const zone_t *zone,
                             const knot_rrset_t *their_soa)
{
	...
	/* TODO: Originally, JK removed locking in his zone-api branch for some reason.
	 * I don't know why since those locks are still in the zone structure.
	 * Figure it out. */
	...
	ret = journal_load_changesets(path, zone, ch, serial_from, serial_to);
...
{
	const knot_pktsection_t *answer = knot_pkt_section(pkt, KNOT_ANSWER);
	return knot_pkt_rr(answer, 0)->type == KNOT_RRTYPE_SOA &&
