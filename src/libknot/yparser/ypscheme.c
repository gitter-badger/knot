...
static void reset_ctx(
	...
	size_t index)
{
	assert(index < YP_MAX_NODE_COUNT);
	...
	yp_check_ctx_t *ctx)
{
	...
	if (parent != NULL) {
		...
		if (parent->item == NULL || parent->item->type != YP_TGRP) {
			return KNOT_YP_EINVAL_ITEM;
		}

		// Check for valid subitem.
	...
	if (node->item->type == YP_TGRP && data_len != 0) {
	...
	const char *data)
{
	...
	reset_ctx(ctx, 0);
	if (key1_len == 0) {
		ret = check_item(key0, key0_len, data, data_len, ctx);
	} else {
		ret = check_item(key0, key0_len, NULL, 0, ctx);
	}
	if (ret != KNOT_EOK) {
		return ret;
	}
	...
	if (id_len != 0) {
		...
		reset_ctx(ctx, 1);
		ret = check_item(name + 1, name[0], id, id_len, ctx);
		if (ret != KNOT_EOK) {
			return ret;
		}
	...
	if (key1_len != 0) {
		reset_ctx(ctx, 1);
		ret = check_item(key1, key1_len, data, data_len, ctx);
		if (ret != KNOT_EOK) {
			return ret;
		}

		// Check for id in key1.
		if (ctx->current != 1) {
			return KNOT_YP_EINVAL_ITEM;
		}
	}
